...
function template_messages_informations_above()
{
	...
	echo '
		<main id="forumposts" class="mpart show_mpart">
...
function template_messages()
{
	...
	while ($message = $controller->{$context['get_message'][1]}())
	{
		...
		if ($settings['show_modify'])
			echo '
						<summary id="modified_', $message['id'], '" class="listlevel1 modified', !empty($message['modified']['name']) ? '"' : ' hide"', '>
...
function template_quickreply_below()
{
	...
	{
		echo '
			<div id="quickreplybox" class="mpart show_mpart">
				<h2 class="category_header category_toggle">
					<i id="qrBig" data-id="quickreplybox" class="chevricon i-chevron-expand"></i>
					<span>
...
function template_display_calendar_above()
{
	...
	echo '
			<section class="linked_events mpart" id="cal_event">
...
function template_pages_and_buttons_above()
{
	...
	template_pagesection('normal_buttons', 'right',NULL,'above mpart','pagesect');
}
...
function template_pages_and_buttons_below()
{
	...
	echo '
			<div id="moderationbuttons" class="hide_30 hamburger_30_target mpart">', template_button_strip($context['mod_buttons'], 'bottom', array('id' => 'moderationbuttons_strip')), '</div>';

	// Show the jump-to box, or actually...let Javascript do it.
	echo '
			<div id="display_jump_to" class="mpart">&nbsp;</div>
			<script>
				aJumpTo[aJumpTo.length] = new JumpTo({
					sContainerId: "display_jump_to",
					sJumpToTemplate: "<h2 class=\"category_header\">', $context['jump_to']['label'], '</h2>%dropdown_list%",
	...
	foreach ($message['attachment'] as $attachment)
	{
		...
		if ($attachment['is_image'])
		{
			if ($attachment['thumbnail']['has_thumb'])
				...
				echo '
											<div class="attachment_image_cont">
												<img class="attachment_image" src="', $attachment['href'], ';image" alt="" title="' . $attachment['name'] . '" />
											</div>';
		}
		elseif (!empty($modSettings['attachmentShowImages']))
			echo '							<div class="attachment_image_cont"><img class="attachment_image" src="', $attachment['href'], ';thumb" alt="" style="max-width:' . $modSettings['attachmentThumbWidth'] . 'px; max-height:' . $modSettings['attachmentThumbHeight'] . 'px;" /></div>';

		echo '
											<a href="', $attachment['href'], '" class="attachment_name" title="' . $attachment['name'] . '">
												<i class="icon icon-small i-paperclip"></i>' . $attachment['name'] . '
											</a>
											<span class="attachment_details">', $attachment['size'], ($attachment['is_image'] ? ', ' . $attachment['real_width'] . 'x' . $attachment['real_height'] . ' <br>' . sprintf($txt['attach_viewed'], $attachment['downloads']) : ' ' . sprintf($txt['attach_downloaded'], $attachment['downloads'])) . '</span>';
...
function c_sidebarbuttons()
{
	global $settings, $context, $txt, $modSettings, $options;
	
	echo '
<div id="mobile_nav">
	<ul>
		<li data-id="forumposts" data-ida="pagesect" data-idb="quickreplybox"><i class="icon i-list icon-big"></i></li>
		<li data-id="display_jump_to"><i class="icon i-jump icon-big"></i></li>
		';

	if($context['is_poll'])
		echo '
		<li data-id="poll"><i class="icon i-poll icon-big"></i></li>';
	
	if(!empty($context['linked_calendar_events']))
		echo '
		<li data-id="cal_event"><i class="icon i-calendar icon-big"></i></li>';
