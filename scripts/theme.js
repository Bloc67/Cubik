$(function() {
	...
	$('#forumposts li.inline_mod_check input[type=checkbox]').change(function(e) {
		$('#moderationbuttons').removeClass('fixedbottom');
		$('#footer_section').removeClass('bigbottom');
		var isChecked = false;
		$('#forumposts li.inline_mod_check input.input_check').each(function(i) {
			if($(this).is(':checked')) {
				isChecked = true;
			} 
		});
		if(isChecked == true) {
			$('#moderationbuttons').addClass('fixedbottom');
		}
	});
